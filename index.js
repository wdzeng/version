...
function getPackageJson() {
    try {
        return JSON.parse(node_fs_1.default.readFileSync('package.json', 'utf8'));
    }
    catch (err) {
        console.trace(err);
        core_1.default.setFailed('Error reading package.json');
        process.exit();
    }
}
...
let outputPrefix = core_1.default.getInput('prefix') || 'false';
if (outputPrefix != 'v' && outputPrefix != '' && outputPrefix != 'false') {
    core_1.default.setFailed('Prefix must be on of "v", "false" or empty string');
    process.exit();
}
const packageJson = getPackageJson();
let version = packageJson.version;
if (!version) {
    core_1.default.setFailed('Version not found');
    process.exit();
}
let versionPrefix = '';
...
if (!isValidSemver(version)) {
    core_1.default.setFailed('Invalid semver: ' + version);
    process.exit();
}
let author = packageJson.author || '';
if (typeof author === 'object') {
    if (!author.name) {
        core_1.default.setFailed('Author name not found');
        process.exit();
    }
...
if (typeof author !== 'string') {
    core_1.default.setFailed('Author must be a string');
    process.exit();
}
const description = packageJson.description || '';
if (typeof description !== 'string') {
    core_1.default.setFailed('Description must be a string');
    process.exit();
}
let license = packageJson.license || '';
if (typeof license === 'object') {
    if (!license.type) {
        core_1.default.setFailed('License type not found');
        process.exit();
    }
    license = license.type;
}
if (typeof license !== 'string') {
    core_1.default.setFailed('License must be a string');
    process.exit();
}
outputPrefix = outputPrefix === 'false' ? versionPrefix : outputPrefix;
core_1.default.setOutput('description', description);
core_1.default.setOutput('license', license);
core_1.default.setOutput('author', author);
const outputVersion = getVersions(version);
core_1.default.setOutput('major', outputPrefix + outputVersion.major);
core_1.default.setOutput('minor', outputPrefix + outputVersion.minor);
core_1.default.setOutput('patch', outputPrefix + outputVersion.patch);
core_1.default.setOutput('version', outputPrefix + outputVersion.version);
core_1.default.setOutput('pre-release', outputVersion.isPreRelease ? 'true' : 'false');
